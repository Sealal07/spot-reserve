...
@router.post('/spots')
def create_spot(db: Session = Depends(get_db), current_user: User = Depends(get_current_user), number=None,
                description=None):
    '''Создание стола'''
    ...
    try:
        ...
        db.add(new_spot)
        db.commit()
        message = ResponseMessage(error=200, message='Стол успешно создан!')
        return message
...
def delete_spot(spot_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    db.delete(del_spot)
    db.commit()
    message = ResponseMessage(error=200, message='Стол успешно удален!')
    return message

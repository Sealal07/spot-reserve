...
def create_booking(
    ...
    # Добавим проверку на время бронирования
    ...
    try:
        if start_time < time_now:
            raise HTTPException(status_code=400, detail='Время\дата начала бронирования не может быть меньше текущего времени\даты!')
        elif end_time <= start_time:
            raise HTTPException(status_code=400, detail='Время\дата завершения бронирования должна быть больше чем время\дата начала бронирования!')
...
def get_my_bookings(user_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    # проверка на существование бронирований
    if current_user:
...
def delete_booking(id:int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    if not current_user or Booking.user_id != current_user.id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail='Вы не можете отменять чужие брони'
        )
    # проверка удаление админом и пользователем, кому принадлежит бронь
    if current_user.role == 'admin' or Booking.user_id == current_user.id:
        db.query(Booking).filter(Booking.id == id).delete()
        db.commit()
    return {'message': 'Успешно удалено'}

    # убираем, т.к. теперь есть проверка на пользователя(выше)
